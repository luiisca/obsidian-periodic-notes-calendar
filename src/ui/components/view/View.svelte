  ...
  let heartbeat = setInterval(() => {
    ...
    if (
      ...
      $todayStore
        .clone()
        .startOf("month")
        .isSame($displayedDateStore.clone().add(1, "month").startOf("month"))
    ) {
    ...
    >
      ...
      >
        <Button
          ...
          class={cn(
            "clickable-icon view-action italic absolute [transform:translateY(-50%)] w-fit cursor-[var(--cursor)] py-3 hover:!bg-transparent",
            Platform.isPhone
