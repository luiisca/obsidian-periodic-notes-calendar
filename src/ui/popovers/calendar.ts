...
import { WindowEventHandler, type TWindowEvents } from '../types';
...
export class CalendarPopoverBehavior extends BaseComponentBehavior {
  ...
  public boundCallbacks = new Map<keyof WindowEventMap, WindowEventHandler<keyof WindowEventMap>>();
  public boundRefElCallback = new Map<keyof WindowEventMap, WindowEventHandler<keyof WindowEventMap>>();

  constructor(
    private params: TCalendarPopoverParams,
  ) {
    super(CALENDAR_POPOVER_ID, params.view);
  ...
  public open() {
    this.refHtmlEl = getRefHtmlEl();
    if (this.refHtmlEl) {
      super.open(this.refHtmlEl, false);
    }
    this.addWindowListeners(this.getWindowEvents(), this.boundCallbacks);
  }
  ...
  public cleanup() {
    this.close();
    const mouseoverCb = this.boundRefElCallback.get('mouseover')
    if (mouseoverCb) {
      this.refHtmlEl?.removeEventListener('mouseover', mouseoverCb);
    }
  ...
  public getWindowEvents(): TWindowEvents {
    return {
      ...
      auxclick: this.handleWindowClick.bind(this),
      keydown: this.handleWindowKeydown.bind(this),
      mouseover: this.handleWindowMouseover.bind(this)
    }
  }
  public handleWindowClick(event: MouseEvent) {
    ...
    if (
      this.opened &&
      Popover.stickerSingleton?.behaviors?.opened &&
  ...
  public handleWindowMouseover(event: MouseEvent) {
    if (get(settingsStore).openPopoverOnRibbonHover) {
      ...
      const isOnlyCalendarPopoverOpen =
        this.opened && !Popover.stickerSingleton?.behaviors?.opened;
