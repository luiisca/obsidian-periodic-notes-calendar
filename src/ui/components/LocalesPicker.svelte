<script lang="ts">
  ...
  const handleGranularityChange = () => {
    formatInputVal =
      getNormalizedPeriodSettings(granularityInputVal).settings.selectedFormat
        .value;
  };
  ...
  const handleAccept = async () => {
    if (parsedDate) {
      ...
      const leaf = PluginService.getPlugin()?.app.workspace.getLeaf(false);
...
<div class="pt-4" bind:this={containerEl}>
  ...
  <div class="modal-button-container mt-3">
    <Button class="cursor-[var(--cursor)]" onclick={handleCancel}
      >Never mind</Button
    >
    <Button
