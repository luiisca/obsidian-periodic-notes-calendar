<script lang="ts">
  ...
  $: {
    ...
    basename = getBasename(value); // returns actual value if nested, i.e.: "[year]-2021/DD-MM-YY" returns "DD-MM-YY"
  }
  ...
  // TODO: remove this fn, error is reactive now so it will clean itself once value is valid again.
  function clearError() {
    error = "";
  }

  // TODO: replace inputEl.value with jsut value and remove onChange method in favor of reactive error value that updates every time value changes
  function onChange() {
    error = validateFormat(inputEl.value, periodicity);
  }
</script>
