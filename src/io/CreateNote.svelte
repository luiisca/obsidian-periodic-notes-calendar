<script lang="ts">
	...
	const dailyNotesPluginEnabled = (<any>window.app).internalPlugins?.getPluginById(
...
{#if periodSettingsType === 'period'}
	Note: Using {periodicity} format <span class="u-pop">{periodSettings.selectedFormat.value}</span>
...
{:else if periodSettingsType === 'default'}
	Note: Using default format <span class="u-pop">{periodSettings.selectedFormat.value}</span>
